...
def rpn_losses_creator(num_anchors, rpn_scores, rpn_coordinates,
                        rpn_labels, rpn_target_anchors, rpn_w_in, rpn_w_out, tag='faster_rcnn_vgg'):

    if tag == 'faster_rcnn_vgg':
    ...
    rpn_reg_loss = smooth_l1_loss(rpn_coordinates, rpn_target_anchors, rpn_w_in, rpn_w_out)
...
# FIXME：这里有问题
def smooth_l1_loss(pred_bboxes, target_anchors, w_in, w_out, sigma=1.0):
    '''
    ...
    reg_loss = tf.reduce_mean(
        tf.reduce_sum(
            w_out * (tf.pow(x, 2) * (sigma / 2.) * sign + (abs_x - (0.5 / sigma)) * (1. - sign)), axis=1))
    return reg_loss
